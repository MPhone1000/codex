...
```bash
gh workflow run internal-rust-release.yml -R MPhone1000/codex -f upstream_tag=rust-vX.Y.Z -f release_ref=candidate/queue/rust-vX.Y.Z -f internal_tag=internal-rust-vX.Y.Z -f publish=true
```

仅进行空跑捆绑：

```bash
gh workflow run internal-rust-release.yml -R MPhone1000/codex -f upstream_tag=rust-vX.Y.Z -f release_ref=candidate/queue/rust-vX.Y.Z -f internal_tag=internal-rust-vX.Y.Z-dryrun -f publish=false
```
...
- 发布存在，且具有预期的标签/版本
- 发布说明包含上游标签和补丁栈
- 发布资产包含内部二进制文件、`codex-package-<target>.tar.gz`、`codex-package_SHA256SUMS`、`config-schema.json`、`install.sh`和`install.ps1`
